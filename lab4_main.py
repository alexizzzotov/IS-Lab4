...
# ввод входного вектора х
x = np.array([5.1, 3.5, 1.4, 0.2])  # setosa
#x = np.array([6.2, 2.9, 4.3, 1.3]) # versicolor
#x = np.array([7.1, 3.0, 5.9, 2.1]) # virginica

# ввод матриц весов и векторов смещения после обучения
...
# функция активации (выбор максимального)
def relu(t): 
    return np.maximum(t, 0)

# функция для получения вектора вероятностей
def softmax(t):
    out = np.exp(t)
    return out / np.sum(out)

# функция для получения вероятностей каждого класса
def predict(x):
    t1 = x @ W1 + b1
    h1 = relu(t1)
    t2 = h1 @ W2 + b2
...
print('Нейронная сеть для классификации ирисов', '\n----------------------------------------')
print('Прогнозированный класс ириса:\n', class_names[pred_class], '\n')
